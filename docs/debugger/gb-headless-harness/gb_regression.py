...
def rom_mode(path):
    """Mirror the BIOS-less loader: CGB-flagged carts run as CGB, else DMG."""
    try:
        with open(path, "rb") as f:
            f.seek(0x143)
            flag = f.read(1)[0]
        ...
        return "dmg"
    return "cgb" if flag in (0x80, 0xC0) else "dmg"
...
def capture(harness, rom, frames, timeout):
    ...
    and some titles fade through blank frames.
    """
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            subprocess.run([harness, raw, str(max(frames) + 20), rom_mode(raw), "noinput",
                            f"fb={fb}"],
...
def main():
    ...
    roms = [r for r in roms if rom_class(r) == "gb"
