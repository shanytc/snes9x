...
static void rerender_line_span (int line, int x0, int x1)
{
	...
	const uint8	savedR0 = IPPU.Red[0], savedG0 = IPPU.Green[0], savedB0 = IPPU.Blue[0];
	{
		...
		PPU.CGDATA[0] = c;
		IPPU.Red[0]   = IPPU.XB[c & 0x1f];
		IPPU.Green[0] = IPPU.XB[(c >> 5) & 0x1f];
		IPPU.Blue[0]  = IPPU.XB[(c >> 10) & 0x1f];
		IPPU.ScreenColors[0] = (uint16) BUILD_PIXEL(IPPU.Red[0], IPPU.Green[0], IPPU.Blue[0]);
	}
