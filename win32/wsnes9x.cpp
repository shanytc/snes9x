                        ...
                        if ((listview_notify->uOldState & LVIS_SELECTED) != (listview_notify->uNewState & LVIS_SELECTED))
                        {
                            ...
                            else
                            {
                                ...
                                if ((listview_notify->uNewState & LVIS_SELECTED))
                                {
                                    ...
                                    std::vector<wchar_t> buf(CHEAT_SIZE);
	...
	case WM_COMMAND:
		{
			...
			case IDC_UPDATE_CHEAT:
				{
					...
					std::string valid_cheat = S9xCheatValidate(codeUtf8);
			...
			case IDC_BUTTON_EDIT_CHEATGROUP:
				{
                    std::wstring editText = GetDlgItemTextWstring(hDlg, IDC_CHEAT_CODE);
					CCheatGroupDialog dialog(editText);
					if (dialog.DoModal(hDlg) == IDOK)
					{
						std::wstring newEditText = dialog.GetCheatCodes();
                        SetDlgItemText(hDlg, IDC_CHEAT_CODE, newEditText.c_str());
					}
				}
				break;
			case IDOK:
...
INT_PTR CALLBACK DlgCheatSearch(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		WinRefreshDisplay();
			if(val_type==0)
				val_type=1;
			ListView_SetExtendedListViewStyle(GetDlgItem(hDlg, IDC_ADDYS), LVS_EX_FULLROWSELECT | LVS_EX_DOUBLEBUFFER);
...
{
    return GetTextWstring(GetDlgItem(hDlg, nIDDlgItem));
}

std::wstring GetTextWstring(HWND hWnd)
{
    int text_length = GetWindowTextLength(hWnd);
    std::wstring edit_text(text_length + 1, 0);
...
void CpuDebugFormatSnes(wchar_t *out, size_t outLen)
{
...
INT_PTR CALLBACK CpuDebugDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        HWND hTabs = GetDlgItem(hDlg, IDC_CPU_DEBUG_TAB);
        TCITEM tie = {};
        tie.mask = TCIF_TEXT;
        tie.pszText = const_cast<wchar_t *>(L"SNES");
        TabCtrl_InsertItem(hTabs, 0, &tie);
        tie.pszText = const_cast<wchar_t *>(L"GB / SGB");
    ...
    case WM_NOTIFY:
        if (((LPNMHDR)lParam)->idFrom == IDC_CPU_DEBUG_TAB &&
            ((LPNMHDR)lParam)->code == TCN_SELCHANGE) {
            // Force immediate refresh on tab change.
            PostMessage(hDlg, WM_TIMER, CPU_DEBUG_TIMER_ID, 0);
            return TRUE;
        }
        break;
