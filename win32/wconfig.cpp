...
#define CATEGORY "Display"
	...
	AddBoolC("GBVideoCamera", Settings.GBVideoCamera, false, "true to feed a connected USB webcam into the Game Boy Camera (Pocket Camera) cart's image sensor");
	AddUIntC("GBVideoCameraIndex", Settings.GBVideoCameraIndex, 0, "index of the selected webcam in the system device list (set via Display Settings > Game Boy Image)");
#undef CATEGORY
#define CATEGORY "Display\\Win"
	...
	AddIntC("Saturation", Settings.Saturation, 0, "saturation adjustment (-100..+100, 0=no change)");
#undef CATEGORY
...
#define CATEGORY "Settings\\Win\\Files"
	...
	AddStringC("Dir:Bios", GUI.BiosDir, _MAX_PATH, ".\\BIOS", "folder the BIOS Manager's Select... dialog starts in; blank or missing = next to the executable");
...
#define	CATEGORY "Sound"
	...
	AddBoolC("SuppressNRxGlitches", Settings.GBSuppressNRxGlitches, false, "Game Boy: hide the volume jump a channel makes when a game rewrites NRx2 while it is still enabled (the DMG/CGB 'zombie' glitch). false (default) = exact hardware behaviour, which clicks on every note in some games");
...
#define	CATEGORY "Hack"
    ...
    AddBoolC("NSSJoypadWatchdog", Settings.NSSJoypadWatchdog, false, "Nintendo Super System: let the supervisor throw a game off the machine when it stops reading the joypads. Off by default because our model of it has a confirmed false positive - Lethal Weapon spends 257 and then 451 frames uploading its sound driver without touching the pads, which is 2 frames past the BIOS's limit, and a reference core agrees the game really does that");
